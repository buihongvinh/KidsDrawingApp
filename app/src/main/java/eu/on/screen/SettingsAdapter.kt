...
class SettingsAdapter(private val mContext: Context, private val mData: List<ListItemModel>) :
    ...
    override fun getView(position: Int, @Nullable convertView: View?, parent: ViewGroup): View {
        val view = convertView ?: LayoutInflater.from(mContext)
            .inflate(eu.on.screen.R.layout.list_item_layout, parent, false)

        val item = mData[position]

        val imageView1 = view.findViewById<ImageView>(eu.on.screen.R.id.sss)
        val aSwitch = view.findViewById<android.widget.Switch>(eu.on.screen.R.id.switch1)
        val title = view.findViewById<TextView>(eu.on.screen.R.id.minimize)
        val description = view.findViewById<TextView>(eu.on.screen.R.id.descript)
